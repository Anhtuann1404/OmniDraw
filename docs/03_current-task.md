...
| :--- | :--- | :--- | :--- |
| **TV4 — Project Lead & Handwriting / CA-VHC Composition Lead** | Bản triển khai nháp PR3 (Diacritic-Aware Trellis DAG) đã lập trên nhánh backup; đang chờ xử lý findings kỹ thuật và nghiệm thu chính thức cùng TV2 | Chờ TV2 recheck & TV4 review | 🟡 Đang review nháp PR3 |
| **TV2 — Stroke Optimization & Path Planning Lead** | Đã ban hành phiếu review E4 (commit `e0c9f91`: `CHANGES_REQUIRED` / `PASS_WITH_CHANGES`); chờ TV4 xử lý immutability, zero-tangent, fail-closed semantics để recheck | Chờ TV4 hoàn thiện | 🟡 E4 Reopened (Changes Required) |
...
- [ ] **BƯỚC C / PR3 — CA-VHC Diacritic-Aware Trellis DAG (Bản triển khai nháp trên backup branch, chờ TV4/TV2 nghiệm thu):** Đã triển khai 5 slices nháp trên nhánh `backup/pr3-implementation`:
  - *Slice 1 (Types, Candidate Generation & Single-Frame Transform):* `DiacriticConfig`, `DiacriticCandidate`, `CompositionState`, `TransitionWeights`, `TransitionCostBreakdown`, `TransitionResult`, `GlyphWorldGeometry`, `NoValidCompositionState`, sinh ứng viên và cắt tỉa cứng hard-invalid ($K_{\text{raw}} \le 9$), affine transform đơn khung.
  - *Slice 2 & 3 (Transition Cost Evaluation & Diacritic-Aware Viterbi DP):* Triển khai `evaluate_composition_transition` tính $J_{\text{lift}} = w_1 D_{\text{penup}} + w_2 N_{\text{lift}}$ ($N_{\text{lift}} = 1$) và $J_{\text{conn}} = w_3 C_{\text{curvature}} + w_4 C_{\text{bridge\_collision}}$; kiểm tra va chạm cầu nối trực tiếp với nét dấu và thân chữ của cả 2 ký tự liền kề; hard reject ($+\infty$) nếu cắt qua dấu; triển khai `optimize_word_composition_dag` giải thuật quy hoạch động Viterbi theo Bellman recurrence, fail-closed an toàn khi không có đường đi hợp lệ.
  - *Slice 4 (Renderer Integration & Double-Render Elimination):* Tích hợp `algorithm_mode="proposed_ca_vhc"` vào `_text_to_strokes_impl` và `text_to_strokes`; tái sử dụng trực tiếp bridge strokes từ Viterbi DP; đặt dấu trực tiếp từ `state.diacritic_candidate` theo vector dịch chuyển thế giới, bỏ qua `generate_accents` hậu kỳ để ngăn chặn triệt để nhân đôi nét dấu; bảo toàn 100% tương thích ngược cho `b3_current_trellis` và 48/48 ASCII baseline fingerprints.
  - *Slice 5 (Vietnamese Acceptance Test Suite):* Tạo `tests/test_ca_vhc_composition_state.py` (10/10 PASS) và `tests/test_pr3_vietnamese_acceptance.py` (8/8 PASS) chứng minh bảo toàn NFD marks, 0 collision trên 10 từ DEV acceptance, độ cao clearance, tính tất định seed, trần số trạng thái $\le 9$ và không duplicate nét dấu; toàn bộ test suite dự án đạt **158/158 tests PASS** (100% PASS). *Mã nguồn đang chờ TV4 và TV2 nghiệm thu chính thức và giải quyết các điều kiện E4 trước khi tích hợp.*
