...
| :--- | :--- | :--- | :--- |
| **TV4 — Project Lead & Handwriting / CA-VHC Composition Lead** | Hoàn tất Strict Validation (34/34 PASS); hoàn tất BƯỚC A (`06_audit_trellis_dag_report.md`); hoàn tất và đóng BƯỚC B (`07_diacritic_aware_state_design.md`) (APPROVED AND CLOSED); hoàn tất BƯỚC C / PR1 (Commit `daca566`: internal experiment metrics, automated runner, CSV schema, DEV fixture 160 ca); hoàn tất Pre-PR3 Acceptance Contract ([`09_pr3_acceptance_criteria.md`](09_pr3_acceptance_criteria.md), khóa 48 ca ASCII, Full automated suite: 67 passed tại lần verification này); chờ TV2 hoàn thành PR2 baseline adapters để mở Entry Gate PR3; tổng hợp và khóa Research Questions (RQ1–RQ3); dựng khung báo cáo Chương 3 & 4 | Không còn blocker phần mềm cho PR1 và Pre-PR3 contract. PR3 chờ PR2 baseline adapters của TV2 và thống nhất interface chuyển tiếp. TV1 corpus freeze và TV3 hardware calibration là downstream validation gates. | 🟡 Đang làm |
...
- [x] **✅ Pre-PR3 Acceptance Contract & Khóa Snapshot ASCII (Docs 09):** Ban hành Hợp đồng Nghiệm thu Kỹ thuật trước PR3 ([`09_pr3_acceptance_criteria.md`](09_pr3_acceptance_criteria.md)), xác lập 4 trạng thái kiểm chứng (`PASS_CURRENT`, `PR2_DEPENDENT`, `PR3_REQUIRED`, `BLOCKED_CALIBRATION`), khóa snapshot 48 trường hợp ma trận ASCII cơ sở ([`tests/fixtures/pr3_ascii_baseline_fingerprints.json`](../tests/fixtures/pr3_ascii_baseline_fingerprints.json)), hoàn thành 6 automated tests kiểm chứng ([`tests/test_pr3_acceptance_baseline.py`](../tests/test_pr3_acceptance_baseline.py)) bảo vệ A1, A2, D1, D2, G1 (`Full automated suite: 67 passed tại lần verification này`); xác lập Entry Gate cho PR3 (chờ PR2 của TV2); FORMAL EXPERIMENT READINESS: NO (Acceptance suite không lựa chọn, iterate, render hoặc ghi nhận bất kỳ case nào từ `BENCHMARK_HOLDOUT_CORPUS_20`).
