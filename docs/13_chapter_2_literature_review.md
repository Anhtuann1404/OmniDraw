...
Với từ gồm $N$ vị trí và tối đa $K$ trạng thái sau pruning ở mỗi vị trí, recurrence đầy đủ xét $O(NK^2)$ cạnh; bộ nhớ của riêng bảng chi phí và backpointer là $O(NK)$. Nếu mỗi cạnh cần kiểm tra bridge với các đoạn polyline, phép thử hình học đơn giản có thể tốn $L_{geom}=O(L_{bridge}L_{glyph})$ cho $L_{bridge}$ đoạn bridge và $L_{glyph}$ đoạn nét liên quan; khi đó cận thời gian là $O(NK^2L_{geom})$, chưa tính sinh ứng viên/transform hoặc geometry cache. Trong thiết kế CA-VHC, một `CompositionState` kết hợp biến thể thân chữ và ứng viên dấu. State cost đo thuộc tính nội tại như legibility, placement và va chạm bên trong; transition cost đo quan hệ giữa hai trạng thái kề nhau như quãng đường pen-up, số lần nhấc bút, curvature và bridge collision. Việc tách hai nhóm chi phí giúp hạn chế double-count và giữ recurrence có thể kiểm tra; số điểm lấy mẫu bridge vẫn cần được đo khi benchmark.
