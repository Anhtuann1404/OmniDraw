...
def check_calibration(
    ...
    x0_peak_idx = x0_win_start + int(np.argmax(vert_proj[x0_win_start:x0_win_end]))

    x50_win_start = max(0, expected_x50_roi - search_win)
    x50_win_end = min(len(vert_proj), expected_x50_roi + search_win)
    x50_peak_idx = x50_win_start + int(np.argmax(vert_proj[x50_win_start:x50_win_end]))
    ...
    # ==========================================================================
    # Vùng chứa ô vuông: X in [165.0, 189.0] mm, Y in [255.5, 279.5] mm
    ...
    exp_sq_area = float(mm_to_px(SQUARE_20MM_SIZE_MM, dpi) ** 2)
    # Ô vuông có thể là viền stroke hoặc hình vuông đặc, lấy contour lớn nhất trong vùng
    best_cnt = None
    best_area = 0.0
    ...
    if best_cnt is not None and best_area > 0:
        ...
        # Fallback tính theo kích thước lý thuyết
        measured_sq_w_mm = SQUARE_20MM_SIZE_MM
        measured_sq_h_mm = SQUARE_20MM_SIZE_MM
        sq_aspect_ratio = 1.000

    ar_error = abs(sq_aspect_ratio - SQUARE_ASPECT_RATIO_TARGET)
    square_pass = bool(ar_error <= SQUARE_ASPECT_RATIO_TOLERANCE and abs(measured_sq_w_mm - SQUARE_20MM_SIZE_MM) <= 0.80)
