...
5. `OmniDraw — CA-VHC Diacritic-Aware State Architecture` - 19 edges
6. `eval_transition()` - 18 edges
7. `optimize_word_dag()` - 18 edges
8. `process()` - 18 edges
9. `OmniDraw — Tài liệu chuẩn giao tiếp giữa các mảng (API/Data Contract)` - 18 edges
10. `2. Bảng đặc tả từng bước trong Call Chain` - 17 edges
...
- `2.1 Nguyên tắc bất biến đối với `GlyphVariant`` --references--> `GlyphVariant`  [INFERRED]
  docs/07_diacritic_aware_state_design.md → backend/handwriting/engine.py
...
Cohesion: 0.10
Nodes (24): audit_vietnamese_lowercase_accents(), generate_accents(), Kiểm định bộ dấu tiếng Việt viết thường cho một Font Pack. Kiểm tra 12 họ…, Sinh các nét dấu Tiếng Việt (Single-stroke Accents) căn chỉnh tự động: - Bố trí…, 1. Bảng danh mục Tests hiện có liên quan đến Handwriting & DAG, 1. Luồng xử lý dấu từ Unicode đến SVG Render, 1. Sơ đồ luồng thực thi (End-to-End Call Chain), 2. Phân loại hai nhóm dấu tiếng Việt trong mã nguồn (+16 more)
...
Cohesion: 0.08
Nodes (34): ExperimentLog, load_prompts_from_docx(), log_to_csv(), main(), process_all_prompts(), PromptData, Đọc file DOCX, trích xuất 15 prompt, OmniDraw — AI Core API Generator Phần mềm: Gọi API sinh ảnh cho 15 prompt thử… (+26 more)
...
### Community 7 - "_run_self_check"
Cohesion: 0.12
Nodes (18): _catmull_rom_cubic_segments(), compute_pixel_to_mm_transform(), get_render_bounds_px(), kinematic_cost(), nearest_neighbor_order(), polyline_to_path_d(), Ponytail runnable check: assert NN + 2-Opt + Or-Opt reduces travel distance., Chiêu 3: Khoang cach Euclid + phat goc quay quan tinh dong hoc: dist + lambda *… (+10 more)
...
### Community 9 - "handwriting_engine.py"
Cohesion: 0.14
Nodes (16): backend_handwriting, OmniDraw - Handwriting Engine Compatibility Shim…, Nội dung thư tay không thể đặt trọn vẹn trong một trang., Nội dung chứa ký tự chưa có glyph trong Font Pack đang chọn., Loại văn bản / thư (letter_type) không được hỗ trợ hoặc không tương thích font…, Phân giải font_id thành (font_pack, render_profile). Không fallback ngầm cho…, Xác thực letter_type và kiểm tra tính tương thích với font_pack_id. -…, resolve_font() (+8 more)
...
Cohesion: 0.23
Nodes (11): dist(), _edge_cost(), or_opt_improve(), px_to_mm(), OmniDraw - Module Thuat toan: Anh -> SVG toi uu duong ve…, Tong quang duong nhac but (khong tinh quang duong ve net), don vi = don vi cua…, Cai tien thu tu bang Or-opt dua tren danh sach hang xom KHONG GIAN (KD-tree),…, Quy doi pixel -> mm. Neu co truyen paper_size_mm, CLAMP ket qua ve dung trong… (+3 more)

### Community 18 - "OmniDraw — Tài liệu chuẩn giao tiếp giữa các mảng (API/Data Contract)"
Cohesion: 0.05
Nodes (37): call_openai_image_api(), Gọi OpenAI Image API (DALL-E 3), Log, OmniDraw — Progress & Decisions Log, 0. Sơ đồ luồng dữ liệu tổng quát, 10. Thay đổi tài liệu, 1. Chuẩn hoá ảnh đầu vào (Người dùng → Giao diện), 2. Giao diện → AI (request sinh ảnh / style transfer) (+29 more)
...
Cohesion: 0.09
Nodes (29): 2.7 Giao diện, Strict Validation & Tích hợp (Đường chạy TV4), Cách nối API thật (đọc kèm `OmniDraw_API_Spec.md`), getSvgContent(), ComicButton(), HardShadowBox(), Logo(), ScreenShell(), ScreenTitle() (+21 more)

### Community 27 - "camera_inspector.py"
Cohesion: 0.16
Nodes (14): argparse, _encode_jpeg(), inspect_paper(), _normalize_paper_rect(), OmniDraw - Module Thị giác Máy tính: Giám sát Giấy & Camera (Closed-Loop…, Chuyển ảnh OpenCV BGR thành chuỗi Base64 Data URL để UI render trực tiếp., Ponytail runnable check: assert synthetic paper detection precision., Chuẩn hóa góc nghiêng [-45, 45] độ và cạnh dài/cạnh ngắn. (+6 more)
...
Cohesion: 0.06
Nodes (59): Any, asyncio, delete_history_item(), format_time_ago(), get_all_history(), init_db(), parse_svg_info(), Tính toán nhanh số nét vẽ, chiều dài và thời gian ước tính từ file SVG nếu… (+51 more)

### Community 45 - "bridge_collision_cost"
Cohesion: 0.18
Nodes (14): audit_font_pack_geometry(), bridge_collision_cost(), point_to_segment_distance(), Kiểm tra hình học glyph dành cho máy vẽ plotter (Geometry Auditor): Phát hiện…, Tính khoảng cách tối thiểu giữa hai stroke (polyline) trong cùng hệ tọa độ…, Tính khoảng cách Euclide ngắn nhất từ điểm p đến đoạn thẳng nối từ a đến b., Kiểm tra giao nhau giữa 2 đoạn thẳng [p1, p2] và [q1, q2] bền vững: - Sử dụng…, Sampled-polyline / segment-distance heuristic: Tính chi phí va chạm hình học… (+6 more)

### Community 48 - "GlyphVariant"
Cohesion: 0.11
Nodes (25): eval_transition(), get_glyph_variants(), GlyphVariant, optimize_word_dag(), Biến thể hình học (Alloglyph Node) trong đồ thị DAG., Tách nét chính (primary_strokes) và nét phụ (secondary_strokes) của glyph: - i,…, Sinh 1-3 biến thể hình học cho ký tự trên đồ thị DAG: - std: Nét chuẩn…, Tính chi phí chuyển trạng thái giữa 2 node theo hàm mục tiêu: J = w1*D_penup +… (+17 more)
...
Cohesion: 0.12
Nodes (14): can_ligature(), find_unsupported_characters(), point_on_segment(), OmniDraw - Module Viết Thư Tay Tự Động (Bio-Inspired Vietnamese Handwriting…, Kiểm tra nội dung văn bản sau chuẩn hóa NFD và trả về danh sách các ký tự chưa…, Kiểm tra điểm p có nằm trên đoạn thẳng [a, b] trong hệ tọa độ 2D (khi p, a, b…, Fallback tương thích ngược., backend_handwriting_font_packs (+6 more)

## Knowledge Gaps
- **207 isolated node(s):** `graphify`, `Usage`, `What graphify is for`, `Step 0 - GitHub repos and multi-path merge (only if a URL or several paths)`, `Step 1 - Ensure graphify is installed` (+202 more)
...
- **Why does `2.4 Khung giải thuật CA-VHC & Tối ưu đường nét (Đường chạy TV4 & TV2)` connect `2.4 Khung giải thuật CA-VHC & Tối ưu đường nét (Đường chạy TV4 & TV2)` to `GlyphVariant`, `Giai đoạn 2 — Phát triển song song theo 4 đường chạy`, `bridge_collision_cost`?**
...
- **Are the 4 inferred relationships involving `_run_self_check()` (e.g. with `_spy_bridge_tracked()` and `_spy_eval()`) actually correct?**
  _`_run_self_check()` has 4 INFERRED edges - model-reasoned connections that need verification._
